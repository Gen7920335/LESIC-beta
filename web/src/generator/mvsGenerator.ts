...
export const LESIC_METADATA_JSON_PREFIX = "; LESIC_METADATA_JSON=";

export function lesicMetadataBlock(cfg: GeneratorConfig, totalLayers = cfg.bands * cfg.layers_per_band, totalHeight = totalLayers * cfg.layer_height) {
  const metadata = {
    app: "LESIC",
    metadata_version: 1,
    output: cfg.output,
    ...
    temp_step: cfg.temp_step,
    layers_per_band: cfg.layers_per_band,
    bed_temp: cfg.bed_temp,
    temp_wait_tolerance: cfg.temp_wait_tolerance,
    ...
    bed_y: cfg.bed_y,
    bed_z: cfg.bed_z,
    square_x: cfg.square_x,
    square_y: cfg.square_y,
    circle_diameter: cfg.circle_diameter,
    zero_angle_deg: cfg.zero_angle_deg,
    clockwise: cfg.clockwise,
    label: cfg.label,
    label_layout: cfg.label_layout,
    label_height: cfg.label_height ?? null,
    label_margin: cfg.label_margin,
    label_speed: cfg.label_speed,
    label_x_scale: cfg.label_x_scale,
    label_stroke_width: cfg.label_stroke_width,
    label_connector_width: cfg.label_connector_width,
    travel_speed: cfg.travel_speed,
    z_travel_speed: cfg.z_travel_speed,
    min_xy_speed: cfg.min_xy_speed,
    max_xy_speed: cfg.max_xy_speed,
    retract: cfg.retract,
    retract_speed: cfg.retract_speed,
    extrusion_multiplier: cfg.extrusion_multiplier,
    motion_accel: cfg.motion_accel,
    motion_velocity: cfg.motion_velocity,
    motion_minimum_cruise_ratio: cfg.motion_minimum_cruise_ratio,
    motion_square_corner_velocity: cfg.motion_square_corner_velocity,
    motion_jerk: cfg.motion_jerk,
    bands: cfg.bands,
  ...
  return [
    "; LESIC_METADATA_BEGIN",
    "; LESIC readable settings:",
    `; printer=${cfg.printer_name} preset=${cfg.printer_preset}`,
    `; temp=${fmt(cfg.start_temp)}-${fmt(cfg.end_temp)}C step=${fmt(cfg.temp_step)}C layers_per_band=${cfg.layers_per_band}`,
    `; mvs=${fmt(cfg.mvs_min)}-${fmt(cfg.mvs_max)}mm3/s nozzle=${fmt(cfg.nozzle_size)}mm layer_height=${fmt(cfg.layer_height)}mm`,
    `; geometry=square(${fmt(cfg.square_x)},${fmt(cfg.square_y)}) circle_diameter=${fmt(cfg.circle_diameter)}mm`,
    `${LESIC_METADATA_JSON_PREFIX}${JSON.stringify(metadata)}`,
    "; LESIC_METADATA_END",
  ];
}
...
function ringLabelMaskClearance(cfg: GeneratorConfig) {
  return labelMaskClearance(cfg) + RING_LABEL_MASK_EXTRA_CLEARANCE;
}
...
export function getPreviewData(cfg: GeneratorConfig): PreviewData {
  ...
  const ringTickMaskSegments = cfg.label ? buildRingMvsTickMaskSegments(cfg) : [];
  const ringLabelMaskSegments = cfg.label ? buildRingMvsLabelMaskSegments(cfg) : [];
  ...
  const ringClearance = ringLabelMaskClearance(cfg);
  const filterLabelCollisions = (segments: TypedSegment[]) => {
    const withoutBottom = filterSegmentsByObstacles(segments, bottomLabelMaskSegments, maskClearance);
    const withoutTicks = filterSegmentsByObstacles(withoutBottom, ringTickMaskSegments, maskClearance);
    return filterSegmentsByObstacles(withoutTicks, ringLabelMaskSegments, ringClearance);
  };
  const brimSegments = filterLabelCollisions(buildBrimSegments(cfg));
...
export function makeGcode(cfg: GeneratorConfig) {
  ...
  const bodySegments = filterLabelCollisions(pts.slice(1).map((p, i) => [pts[i], p, "stroke"] as TypedSegment));
  const brimLoops = brimRadii(radius, cfg.line_width).map((item) => {
    ...
    return {
      ...item,
      segments: filterLabelCollisions(brimPoints.slice(1).map((p, i) => [brimPoints[i], p, "stroke"] as TypedSegment)),
    };
  });
